...
__webpack_require__.d(__webpack_exports__, {
  "Collection": function() { return /* reexport */ Collection; },
  "FetchServer": function() { return /* reexport */ FetchServer; },
  "Server": function() { return /* reexport */ Server; },
  "Single": function() { return /* reexport */ Single; },
  "default": function() { return /* binding */ FakeRest; }
});
...
var matches = __webpack_require__(6410);
...
var getArrayOfObjectsPaths = function getArrayOfObjectsPaths(keyParts, item) {
  return keyParts.reduce(function (acc, key, index) {
    ...
    var keyToArray = keyParts.slice(0, index + 1).join('.');
    var keyToItem = keyParts.slice(index + 1).join('.');
    var itemValue = get_default()(item, keyToArray);
    ...
    return Array.isArray(itemValue) && index < keyParts.length - 1 ? [keyToArray, keyToItem] : undefined;
  }, undefined);
};
var getSimpleFilter = function getSimpleFilter(key, value) {
  if (key.indexOf('_q') !== -1) {
    // text search
    var realKey = key.replace(/(_q)$/, '');
    var regex = new RegExp(value, 'i');
    return function (item) {
      var _get;
      return ((_get = get_default()(item, realKey)) === null || _get === void 0 ? void 0 : _get.match(regex)) !== null;
    };
  }
  if (key.indexOf('_lte') !== -1) {
    // less than or equal
    var _realKey = key.replace(/(_lte)$/, '');
    return function (item) {
      return get_default()(item, _realKey) <= value;
    };
  }
  if (key.indexOf('_gte') !== -1) {
    // less than or equal
    var _realKey2 = key.replace(/(_gte)$/, '');
    return function (item) {
      return get_default()(item, _realKey2) >= value;
    };
  }
  if (key.indexOf('_lt') !== -1) {
    // less than or equal
    var _realKey3 = key.replace(/(_lt)$/, '');
    return function (item) {
      return get_default()(item, _realKey3) < value;
    };
  }
  if (key.indexOf('_gt') !== -1) {
    // less than or equal
    var _realKey4 = key.replace(/(_gt)$/, '');
    return function (item) {
      return get_default()(item, _realKey4) > value;
    };
  }
  if (key.indexOf("_neq_any") !== -1) {
    // not equal to any
    var _realKey5 = key.replace(/(_neq_any)$/, "");
    var finalValue = Array.isArray(value) ? value : [value];
    return function (item) {
      return finalValue.every(function (val) {
        return get_default()(item, _realKey5) != val;
      });
    };
  }
  if (key.indexOf('_neq') !== -1) {
    // not equal
    var _realKey6 = key.replace(/(_neq)$/, '');
    return function (item) {
      return get_default()(item, _realKey6) != value;
    };
  }
  if (key.indexOf("_eq_any") !== -1) {
    // equal any 
    var _realKey7 = key.replace(/(_eq_any)$/, "");
    var _finalValue = Array.isArray(value) ? value : [value];
    return function (item) {
      return _finalValue.some(function (val) {
        return get_default()(item, _realKey7) == val;
      });
    };
  }
  if (key.indexOf('_eq') !== -1) {
    // equal
    var _realKey8 = key.replace(/(_eq)$/, '');
    return function (item) {
      return get_default()(item, _realKey8) == value;
    };
  }
  if (key.indexOf('_inc_any') !== -1) {
    // include any
    var _realKey9 = key.replace(/(_inc_any)$/, "");
    var _finalValue2 = Array.isArray(value) ? value : [value];
    return function (item) {
      return _finalValue2.some(function (val) {
        return get_default()(item, _realKey9).includes(val);
      });
    };
  }
  if (key.indexOf('_inc') !== -1) {
    // includes all
    var _realKey10 = key.replace(/(_inc)$/, "");
    var _finalValue3 = Array.isArray(value) ? value : [value];
    return function (item) {
      return _finalValue3.every(function (val) {
        return get_default()(item, _realKey10).includes(val);
      });
    };
  }
  if (key.indexOf('_ninc_any') !== -1) {
    // does not include any
    var _realKey11 = key.replace(/(_ninc_any)$/, "");
    var _finalValue4 = Array.isArray(value) ? value : [value];
    return function (item) {
      return _finalValue4.every(function (val) {
        return !get_default()(item, _realKey11).includes(val);
      });
    };
  }
  if (Array.isArray(value)) {
    return function (item) {
      if (Array.isArray(get_default()(item, key))) {
        // array filter and array item value: where all items in values
        return every(value, function (v) {
          return some(get_default()(item, key), function (itemValue) {
            return itemValue == v;
          });
        });
      }
      ...
      return value.filter(function (v) {
        return v == get_default()(item, key);
      }).length > 0;
    };
  }
  if (_typeof(value) === 'object') {
    return function (item) {
      return matches_default()(value)(get_default()(item, key));
    };
  }
  return function (item) {
    if (Array.isArray(get_default()(item, key)) && typeof value == 'string') {
      // simple filter but array item value: where value in item
      return get_default()(item, key).indexOf(value) !== -1;
    }
    if (typeof get_default()(item, key) == 'boolean' && typeof value == 'string') {
      // simple filter but boolean item value: boolean where
      return get_default()(item, key) == (value === 'true' ? true : false);
    }
    // simple filter
    return get_default()(item, key) == value;
  };
};
function filterItems(items, filter) {
  ...
  if (filter instanceof Object) {
    ...
    var filterFunctions = Object.keys(filter).map(function (key) {
      if (key === 'q') {
        var regex = new RegExp(filter.q, "i");
        var filterWithQuery = function filterWithQuery(item) {
      ...
      if (keyParts.length > 1) {
        return function (item) {
          var arrayOfObjectsPaths = getArrayOfObjectsPaths(keyParts, item);
          if (arrayOfObjectsPaths) {
            var _arrayOfObjectsPaths = _slicedToArray(arrayOfObjectsPaths, 2),
              arrayPath = _arrayOfObjectsPaths[0],
              valuePath = _arrayOfObjectsPaths[1];
            // Check wether any item in the array matches the filter
            var filteredArrayItems = filterItems(get_default()(item, arrayPath), _defineProperty({}, valuePath, value));
...
var Collection = /*#__PURE__*/function () {
  function Collection() {
    ...
    var identifierName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'id';
    _classCallCheck(this, Collection);
    _defineProperty(this, "sequence", 0);
    _defineProperty(this, "items", []);
    _defineProperty(this, "server", null);
    _defineProperty(this, "name", null);
    _defineProperty(this, "identifierName", 'id');
  ...
  _createClass(Collection, [{
    ...
    value: function _oneToManyEmbedder(resourceName) {
      ...
      return function (item) {
        ...
        if (!otherCollection) throw new Error("Can't embed a non-existing collection ".concat(resourceName));
    ...
    value: function getAll(query) {
      ...
      if (query) {
        ...
        }); // clone item to avoid returning the original
    ...
    value: function getOne(identifier, query) {
      ...
      if (index === -1) {
        throw new Error("No item with identifier ".concat(identifier));
      }
    ...
    value: function addOne(item) {
      ...
      if (identifier !== undefined) {
        if (this.getIndex(identifier) !== -1) {
          throw new Error("An item with the identifier ".concat(identifier, " already exists"));
        } else {
...
;// CONCATENATED MODULE: ./src/Single.js
...
function Single_createClass(Constructor, protoProps, staticProps) { if (protoProps) Single_defineProperties(Constructor.prototype, protoProps); if (staticProps) Single_defineProperties(Constructor, staticProps); Object.defineProperty(Constructor, "prototype", { writable: false }); return Constructor; }
function Single_defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
var Single = /*#__PURE__*/function () {
  function Single(obj) {
    Single_classCallCheck(this, Single);
    Single_defineProperty(this, "obj", null);
    Single_defineProperty(this, "server", null);
    Single_defineProperty(this, "name", null);
...
;// CONCATENATED MODULE: ./src/parseQueryString.js
function parseQueryString_slicedToArray(arr, i) { return parseQueryString_arrayWithHoles(arr) || parseQueryString_iterableToArrayLimit(arr, i) || parseQueryString_unsupportedIterableToArray(arr, i) || parseQueryString_nonIterableRest(); }
function parseQueryString_nonIterableRest() { throw new TypeError("Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); }
function parseQueryString_unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return parseQueryString_arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return parseQueryString_arrayLikeToArray(o, minLen); }
function parseQueryString_arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) { arr2[i] = arr[i]; } return arr2; }
function parseQueryString_iterableToArrayLimit(arr, i) { var _i = arr == null ? null : typeof Symbol !== "undefined" && arr[Symbol.iterator] || arr["@@iterator"]; if (_i == null) return; var _arr = []; var _n = true; var _d = false; var _s, _e; try { for (_i = _i.call(arr); !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"] != null) _i["return"](); } finally { if (_d) throw _e; } } return _arr; }
function parseQueryString_arrayWithHoles(arr) { if (Array.isArray(arr)) return arr; }
  ...
  queryElements.map(function (queryElement) {
    if (queryElement.indexOf('=') === -1) {
      ...
      var _queryElement$split = queryElement.split('='),
        _queryElement$split2 = parseQueryString_slicedToArray(_queryElement$split, 2),
        key = _queryElement$split2[0],
        value = _queryElement$split2[1];
...
function _createForOfIteratorHelper(o, allowArrayLike) { var it = typeof Symbol !== "undefined" && o[Symbol.iterator] || o["@@iterator"]; if (!it) { if (Array.isArray(o) || (it = Server_unsupportedIterableToArray(o)) || allowArrayLike && o && typeof o.length === "number") { if (it) o = it; var i = 0; var F = function F() {}; return { s: F, n: function n() { if (i >= o.length) return { done: true }; return { done: false, value: o[i++] }; }, e: function e(_e) { throw _e; }, f: F }; } throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); } var normalCompletion = true, didErr = false, err; return { s: function s() { it = it.call(o); }, n: function n() { var step = it.next(); normalCompletion = step.done; return step; }, e: function e(_e2) { didErr = true; err = _e2; }, f: function f() { try { if (!normalCompletion && it.return != null) it.return(); } finally { if (didErr) throw err; } } }; }
function Server_unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return Server_arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return Server_arrayLikeToArray(o, minLen); }
function Server_arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) { arr2[i] = arr[i]; } return arr2; }
...
function Server_defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
...
var Server = /*#__PURE__*/function () {
  function Server() {
    ...
    Server_defineProperty(this, "baseUrl", null);
    Server_defineProperty(this, "loggingEnabled", false);
    Server_defineProperty(this, "defaultQuery", function () {});
    Server_defineProperty(this, "batchUrl", null);
    Server_defineProperty(this, "collections", {});
    Server_defineProperty(this, "singles", {});
    Server_defineProperty(this, "requestInterceptors", []);
    Server_defineProperty(this, "responseInterceptors", []);
  ...
  Server_createClass(Server, [{
    ...
    value: function handle(request) {
      ...
      var _iterator = _createForOfIteratorHelper(this.getSingleNames()),
        _step;
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var _name = _step.value;
          var _matches = request.url.match(new RegExp("^" + this.baseUrl + "\\/(" + _name + ")(\\/?.*)?$"));
          if (!_matches) continue;
          if (request.method == "GET") {
            try {
              var _item4 = this.getOnly(_name);
              return this.respond(_item4, null, request);
            } catch (error) {
          ...
          if (request.method == "PUT") {
            try {
              var _item5 = this.updateOnly(_name, request.json);
              return this.respond(_item5, null, request);
            } catch (error) {
              return request.respond(404);
            }
          }
          if (request.method == "PATCH") {
            try {
              var _item6 = this.updateOnly(_name, request.json);
              return this.respond(_item6, null, request);
            } catch (error) {
              return request.respond(404);
            }
          }
      ...
      var matches = request.url.match(new RegExp("^" + this.baseUrl + "\\/([^\\/?]+)(\\/(\\d+))?(\\?.*)?$"));
      if (!matches) return;
      var name = matches[1];
      var params = Object.assign({}, this.defaultQuery(name), request.params);
      if (!matches[2]) {
        if (request.method == "GET") {
          if (!this.getCollection(name)) {
            return;
          }
          var count = this.getCount(name, params.filter ? {
            filter: params.filter
          } : {});
          var items, contentRange, status;
          if (count > 0) {
            ...
            contentRange = "items ".concat(first, "-").concat(last, "/").concat(count);
        ...
        if (request.method == "POST") {
          var newResource = this.addOne(name, request.json);
          var newResourceURI = this.baseUrl + "/" + name + "/" + newResource[this.getCollection(name).identifierName];
          return this.respond(newResource, {
        ...
        if (request.method == "GET") {
          try {
            var item = this.getOne(name, id, params);
        ...
        if (request.method == "PUT") {
          try {
            var _item = this.updateOne(name, id, request.json);
            return this.respond(_item, null, request);
          } catch (error) {
            return request.respond(404);
          }
        }
        if (request.method == "PATCH") {
          try {
            var _item2 = this.updateOne(name, id, request.json);
            return this.respond(_item2, null, request);
          } catch (error) {
            return request.respond(404);
          }
        }
        if (request.method == "DELETE") {
          try {
            var _item3 = this.removeOne(name, id);
            return this.respond(_item3, null, request);
          } catch (error) {
            return request.respond(404);
          }
        }
...
;// CONCATENATED MODULE: ./src/FetchServer.js
function FetchServer_typeof(obj) { "@babel/helpers - typeof"; return FetchServer_typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (obj) { return typeof obj; } : function (obj) { return obj && "function" == typeof Symbol && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }, FetchServer_typeof(obj); }
function FetchServer_createForOfIteratorHelper(o, allowArrayLike) { var it = typeof Symbol !== "undefined" && o[Symbol.iterator] || o["@@iterator"]; if (!it) { if (Array.isArray(o) || (it = FetchServer_unsupportedIterableToArray(o)) || allowArrayLike && o && typeof o.length === "number") { if (it) o = it; var i = 0; var F = function F() {}; return { s: F, n: function n() { if (i >= o.length) return { done: true }; return { done: false, value: o[i++] }; }, e: function e(_e) { throw _e; }, f: F }; } throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); } var normalCompletion = true, didErr = false, err; return { s: function s() { it = it.call(o); }, n: function n() { var step = it.next(); normalCompletion = step.done; return step; }, e: function e(_e2) { didErr = true; err = _e2; }, f: function f() { try { if (!normalCompletion && it.return != null) it.return(); } finally { if (didErr) throw err; } } }; }
function FetchServer_unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return FetchServer_arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return FetchServer_arrayLikeToArray(o, minLen); }
function FetchServer_arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) { arr2[i] = arr[i]; } return arr2; }
function FetchServer_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
function FetchServer_defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }
function FetchServer_createClass(Constructor, protoProps, staticProps) { if (protoProps) FetchServer_defineProperties(Constructor.prototype, protoProps); if (staticProps) FetchServer_defineProperties(Constructor, staticProps); Object.defineProperty(Constructor, "prototype", { writable: false }); return Constructor; }
...
function _possibleConstructorReturn(self, call) { if (call && (FetchServer_typeof(call) === "object" || typeof call === "function")) { return call; } else if (call !== void 0) { throw new TypeError("Derived constructors may only return object or undefined"); } return _assertThisInitialized(self); }
function _assertThisInitialized(self) { if (self === void 0) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return self; }
function _isNativeReflectConstruct() { if (typeof Reflect === "undefined" || !Reflect.construct) return false; if (Reflect.construct.sham) return false; if (typeof Proxy === "function") return true; try { Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {})); return true; } catch (e) { return false; } }
function _getPrototypeOf(o) { _getPrototypeOf = Object.setPrototypeOf ? Object.getPrototypeOf.bind() : function _getPrototypeOf(o) { return o.__proto__ || Object.getPrototypeOf(o); }; return _getPrototypeOf(o); }


var FetchServer = /*#__PURE__*/function (_Server) {
  _inherits(FetchServer, _Server);
  var _super = _createSuper(FetchServer);
  function FetchServer() {
    FetchServer_classCallCheck(this, FetchServer);
    return _super.apply(this, arguments);
  }
  FetchServer_createClass(FetchServer, [{
    key: "decode",
    ...
    value: function handle(req, opts) {
      ...
      return this.decode(req, opts).then(function (request) {
        ...
        // Handle Single Objects
        var _iterator = FetchServer_createForOfIteratorHelper(_this2.getSingleNames()),
          _step;
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            var name = _step.value;
            var matches = request.url.match(new RegExp('^' + _this2.baseUrl + '\\/(' + name + ')(\\/?.*)?$'));
            if (!matches) continue;
            if (request.method == 'GET') {
              try {
                response.body = _this2.getOnly(name);
              } catch (error) {
                reponse.status = 404;
              }
            ...
            if (request.method == 'PUT') {
              try {
                response.body = _this2.updateOnly(name, request.requestJson);
              } catch (error) {
                reponse.status = 404;
              }
              return _this2.respond(response, request);
        ...
        var _iterator2 = FetchServer_createForOfIteratorHelper(_this2.getCollectionNames()),
          _step2;
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
            var _name = _step2.value;
            var _matches = request.url.match(new RegExp('^' + _this2.baseUrl + '\\/(' + _name + ')(\\/(\\d+))?(\\?.*)?$'));
            if (!_matches) continue;
            var params = Object.assign({}, _this2.defaultQuery(_name), request.params);
            if (!_matches[2]) {
              if (request.method == 'GET') {
                var count = _this2.getCount(_name, params.filter ? {
                  filter: params.filter
                } : {});
                if (count > 0) {
                  var items = _this2.getAll(_name, params);
                  var first = params.range ? params.range[0] : 0;
                  var last = params.range ? Math.min(items.length - 1 + first, params.range[1]) : items.length - 1;
                  response.body = items;
                  response.headers['Content-Range'] = "items ".concat(first, "-").concat(last, "/").concat(count);
              ...
              if (request.method == 'POST') {
                var newResource = _this2.addOne(_name, request.requestJson);
                var newResourceURI = _this2.baseUrl + '/' + _name + '/' + newResource[_this2.getCollection(_name).identifierName];
              ...
              if (request.method == 'GET') {
                try {
                  response.body = _this2.getOne(_name, id, params);
                } catch (error) {
                  response.status = 404;
                }
                return _this2.respond(response, request);
              ...
              if (request.method == 'PUT') {
                try {
                  response.body = _this2.updateOne(_name, id, request.requestJson);
                } catch (error) {
                  response.status = 404;
                }
                return _this2.respond(response, request);
              ...
              if (request.method == 'DELETE') {
                try {
                  response.body = _this2.removeOne(_name, id);
                } catch (error) {
                  response.status = 404;
                }
                return _this2.respond(response, request);
...
;// CONCATENATED MODULE: ./src/FakeRest.js





/* harmony default export */ var FakeRest = ({
  Server: Server,
